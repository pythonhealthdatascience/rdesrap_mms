...
print(table_latex)
print(table_latex, file = file.path(output_dir, "scenario_nurse_util.tex"))
```
...
print(sensitivity_table_latex,
      file = file.path(output_dir, "sensitivity_consult_time"))
```

## Example run with logs

The `verbose` parameter set in our `defaults()` function is used to control whether simmer runs the model with `verbose` set to TRUE or FALSE.
...
```{r}
param <- defaults()
param[["param.data_collection_period"]] <- 100L
param[["number_of_runs"]] <- 1L
param[["cores"]] <- 0L
param[["verbose"]] <- TRUE

verbose_run <- model(run_number = 0, param = param)
```
